...
const keepItSecretKeepItSafe = () => {
  ...
  // 1. create an empty div with an id of 'the-ring'
    ...
    $hobbitArray.eq(0).append($theRing);
    //when i click the button his name disappears, just like in the movie. not sure if that's right thought.
...
const makeBaddies = () => {
  ...
  for(let i = 0; i < baddies.length; i++) {
    ...
    const $baddieNames = $('<li>').addClass('baddy').text(baddies[i]);
    $baddieList.append($baddieNames);
...
for(let i=0; i < buddies.length; i++) {
    ...
    const $buddieNames = $('<li>').addClass('buddy').text(buddies[i]);
    $buddyList.append($buddieNames);
...
const theBalrog = () => {
  ...
  const $buddies = $('.buddy').eq(0).text('Gandalf the White').addClass("the-white");
  console.log($buddies);
...
const itsDangerousToGoAlone = () => {
  ...
  const $removeFrodo = $('#the-fellowship').children().eq(6);
  $('#Mordor').append($removeFrodo);
  const $removeSam = $('#the-fellowship').children().eq(6);
  $('#Mordor').append($removeSam)
  console.log($removeSam);
